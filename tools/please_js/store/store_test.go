...
func pkg(t *testing.T, root, entry, name, version string, deps ...string) store.Source {
	t.Helper()
	...
	return store.Source{
		Dir:  dir,
		Meta: store.Meta{Package: name, Version: version, Name: entry, Deps: deps},
	}
}

func alias(entry, name, of string) store.Source {
	return store.Source{Meta: store.Meta{Package: name, Name: entry, AliasOf: of}}
}
...
func TestScopedAndUnscopedLinksResolve(t *testing.T) {
	...
	if err := store.Build(tree, sources, []string{"react_18_3_1", "scope_thing_1_0_0"}); err != nil {
		t.Fatal(err)
	}
...
func TestDepsResolveFromInsideTheirPackage(t *testing.T) {
	...
	sources := []store.Source{
		pkg(t, root, "react_dom_18_3_1", "react-dom", "18.3.1", "react_18_3_1", "scope_thing_1_0_0"),
		pkg(t, root, "react_18_3_1", "react", "18.3.1"),
		pkg(t, root, "scope_thing_1_0_0", "@scope/thing", "1.0.0"),
	}
	if err := store.Build(tree, sources, []string{"react_dom_18_3_1"}); err != nil {
		t.Fatal(err)
	}
...
func TestCyclicDependenciesAreFine(t *testing.T) {
	...
	sources := []store.Source{
		pkg(t, root, "babel_core", "@babel/core", "7.29.0", "babel_helper"),
		pkg(t, root, "babel_helper", "@babel/helper-module-transforms", "7.28.6", "babel_core"),
	}
	if err := store.Build(tree, sources, []string{"babel_core"}); err != nil {
		t.Fatal(err)
	}
	both := [][2]string{
...
func TestAliasRebindsWithoutDuplicating(t *testing.T) {
	root := t.TempDir()
	tree := filepath.Join(root, "node_modules")
	...
	sources := []store.Source{
		...
		alias("aspect_c1", "@aspect-test/c1", "aspect_c_1_0_0"),
	}
	if err := store.Build(tree, sources, []string{"aspect_c_2_0_2", "aspect_c1"}); err != nil {
		t.Fatal(err)
	}
	...
	// The alias must not have produced a second copy of the package.
	if _, err := os.Stat(filepath.Join(tree, store.StoreRoot, store.StoreDir("aspect_c1"))); err == nil {
		t.Error("the alias made its own store entry; the package is no longer a singleton")
	}
}
...
func TestMissingClosureMemberIsAnError(t *testing.T) {
	root := t.TempDir()
	sources := []store.Source{
		pkg(t, root, "react_dom", "react-dom", "18.3.1", "react_18_3_1"),
	}
	err := store.Build(filepath.Join(root, "node_modules"), sources, []string{"react_dom"})
	if err == nil {
